...
class MemoryPreparer(Sequence):

    def __init__(self, window=False, batch_size=128, window_size = 2,  n_time_col=7, columns_to_drop = ['SENDER_BIC_8','RECEIVER_BIC_8'],tick=False):
        self.window = window
    ...
    def drop_specific_columns(self, df):
        ...
        This function was designed because at this point the aggregate still keeps the BIC
        and payment type columns for consistency. 
        Parameters:
        df (pd.DataFrame): dataframe
        columns_to_drop (list): list op column names which have to be dropped from df
        """
    ...
    def sort_by_time(self, df):
        return df.sort_values(by=['YEAR', 'MONTH', 'DAY', 'HOURS', 'MINUTES', 'SECONDS'])
    ...
    def make_matrix(self, df):
        ...
        df (pd.DataFrame) : Dataframe with the following columns in order:
                            YEAR, MONTH, WEEKNUMBER, DAY, HOURS, MINUTES, SECONDS,
                            MATRIX_INDEX_AMOUNT, MATRIX_INDEX_COUNT, 
                            AMOUNT_OF_TRANSACTION, NUMBER_OF_TRANSACTIONS.
        n_time_col (int) : Number of time stamp columns at the start of the matrix.    
        """
        max_index = int(df['MATRIX_INDEX_COUNT'].max())
        empty_array = np.zeros(max_index + self.n_time_col + 1)
        empty_array[0:self.n_time_col] = 100 # to make sure there is not accidental first match

        big_matrix = empty_array.copy()
        matrix_row = empty_array.copy()

        for df_row in np.array(df):
            if (matrix_row[0:self.n_time_col] != df_row[0:self.n_time_col]).any():
                big_matrix = np.vstack((big_matrix,matrix_row))
                matrix_row = empty_array.copy()
                matrix_row[0:self.n_time_col] = df_row[0:self.n_time_col]

            matrix_row[int(df_row[self.n_time_col + 0]) + self.n_time_col] = df_row[self.n_time_col + 2]
            matrix_row[int(df_row[self.n_time_col + 1]) + self.n_time_col] = df_row[self.n_time_col + 3]
    ...
    def make_windowed_array(self, matrix_2d):
        """Transforms a pandas dataframe of shape (n_samples, n_features) to an np.array
        of shape (n_samples, window_size, n_features). This is the output necesarry for the 
        LSTM and GRU models.
        Parameters:
        df (pd.DataFrame): dataframe
        ...
        if self.window_size > matrix_2d.shape[0]:
            raise ValueError("window_size cannot be larger than the number of samples")
        ...
        for i in range(n_samples):
            new_array[i] = old_array[i : self.window_size + i, :].reshape(1, self.window_size, n_features)

        return new_array
    
    def __getitem__(self, idx):
        start = idx * self.batch_size 
        stop = (idx +1)* self.batch_size if ((idx +1)* self.batch_size) <= len(self.x) else len(self.x)    
